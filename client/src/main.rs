...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        // Temporary brush UI (replaced in F3): TAB cycles unlocked hues,
        // +/- adjusts value. Saturation/hue validation happens server-side.
        if let Some(me) = me {
            let my_hues: Vec<u16> = ctx.db.inventory().iter().filter(|i| i.owner == me).map(|i| i.hue).collect();
            let user = ctx.db.user().identity().find(&me);
            if rl.is_key_pressed(KeyboardKey::KEY_TAB) && !my_hues.is_empty() {
                inventory_idx = (inventory_idx + 1) % my_hues.len();
                let (sat, val) = user.as_ref().map_or((40, 100), |u| (u.sat, u.val));
                let _ = ctx.reducers.set_brush(my_hues[inventory_idx], sat, val);
            }
            if let Some(u) = &user {
                if rl.is_key_pressed(KeyboardKey::KEY_EQUAL) || rl.is_key_pressed(KeyboardKey::KEY_KP_ADD) {
                    let _ = ctx.reducers.set_brush(u.hue, u.sat, (u.val + 5).min(100));
                }
                if rl.is_key_pressed(KeyboardKey::KEY_MINUS) || rl.is_key_pressed(KeyboardKey::KEY_KP_SUBTRACT) {
                    let _ = ctx.reducers.set_brush(u.hue, u.sat, u.val.saturating_sub(5));
                }
            }
        }
...

        {
            ...
            // Hover highlight: always-visible translucent overlay on the
            // cell under the mouse, regardless of its underlying color.
        ...
        // All cursors are the same screen-space pointer shape (constant
        // size regardless of zoom), just in each player's own brush color.
        for &(screen, color) in &other_cursors {
            world::draw_cursor(&mut d, screen, color);
        }
        if let Some(me) = me {
            let (hue, sat, val) = ctx.db.user().identity().find(&me).map_or((0, 40, 100), |u| (u.hue, u.sat, u.val));
        ...
        d.draw_text("hexaworld", 10, 10, 20, Color::RAYWHITE);
        d.draw_text(
            &format!("players online {online} / {total}    TAB: cycle hue   +/-: value   SHIFT+drag/middle: pan   wheel: zoom"),
            10,
            34,
            14,
            Color::LIGHTGRAY,
